...
class Ui_Recent_View_Action_Panel(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setObjectName("Ui_Recent_View_Action_Panel")
        self.load_widgets()
        self.set_layout()
        self.load_widget_assets()

        with open("src/genlore/ui_modules/recent_view/ui_action_panel.qss", "r") as qss_file:
    ...
    def load_widgets(self):
        ...
        self.recent_view_btn.setObjectName("recent_view_btn")
    ...
    def load_widget_assets(self):
        ...
        # mask = logo_icon.createMaskFromColor(QColor("white"),Qt.MaskOutColor)
        # logo_icon.fill(QColor("#A5C9CA"))
        # logo_icon.setMask(mask)
        # self.logo.setPixmap(logo_icon.scaled(self.logo.width(),self.height(),Qt.KeepAspectRatio))
        # self.logo.setAlignment(Qt.AlignHCenter)

        recent_view_icon = QPixmap("icons/rotate-left.svg")
        mask = recent_view_icon.createMaskFromColor(
            QColor("transparent"), Qt.MaskInColor)
        recent_view_icon.fill(QColor("#F3F3F3"))
...
if __name__ == "__main__":
    app = QApplication()
    window = Ui_Recent_View_Action_Panel()
    window.show()
    app.exec()

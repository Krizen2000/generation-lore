...
class Ui_Recent_View_Application_Panel(QWidget):
    def __init__(self, parent=None):
        ...
        with open("src/genlore/ui_modules/recent_view/ui_application_panel.qss", "r") as qss_file:
    ...
    def load_widgets(self):
        ...
        self.exit_btn.clicked.connect(
            lambda: self.mapToGlobal(QPoint(500, 500)))
        ...
        self.view_name = QLabel(parent=self, text="Recent")
    ...
    #         self.pressing = True
    #         self.prev_coords = event.pos()
    #     def mouse_move_handler(self, event):
    #         if self.pressing:
    #             self.window.move((event.globalX()-self.prev_coords.x()),(event.globalY()-self.prev_coords.y()))
    #     def mouse_release_handler(event):
    #         self.pressing = False
    #     self.title_bar.pressed.connect(lambda event: mouse_press_handler(event))
    #     self.mouseMove.connect(lambda event: mouse_move_handler(event))
    #     QWidget.connect(self.title_bar, self.title_bar.mouseMoveEvent, self,lambda event: mouse_move_handler(event))
    #     self.title_bar.released.connect(lambda event: mouse_release_handler(event))
...
if __name__ == "__main__":
    app = QApplication()
    window = Ui_Recent_View_Application_Panel()
    window.show()
    app.exec()

...
def load_qlist_items(cls: Ui_Recent_View_Application_Panel):
    db_context = Db_Module.get_instance()
    story_entries = db_context.get_recent_entries("STORY")
    summary_entries = db_context.get_recent_entries("SUMMARY")
    ...
    for story_entry in story_entries:
        # ? Need to make changes in db_module

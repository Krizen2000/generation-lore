...
class Ui_Doc_Obs_View_Action_Panel(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setObjectName("Ui_Doc_Obs_View_Action_Panel")
        self.load_widgets()
        self.set_layout()
        self.load_widget_assets()

        with open("src/genlore/ui_modules/doc_obs_view/ui_action_panel.qss", "r") as qss_file:
    ...
    def load_widget_assets(self):
        ...
        mask = clipboard_icon.createMaskFromColor(
            QColor("transparent"), Qt.MaskInColor)
        clipboard_icon.fill(QColor("#FE5D9F"))
        clipboard_icon.setMask(mask)
        self.clipboard_btn.setIcon(clipboard_icon)

        save_icon = QPixmap("icons/floppy-disk.svg")
        mask = save_icon.createMaskFromColor(
            QColor("transparent"), Qt.MaskInColor)
        save_icon.fill(QColor("#FE5D9F"))
        save_icon.setMask(mask)
        self.save_btn.setIcon(save_icon)

        favourite_icon = QPixmap("icons/star.svg")
        mask = favourite_icon.createMaskFromColor(
            QColor("transparent"), Qt.MaskInColor)
        favourite_icon.fill(QColor("#FE5D9F"))
...
if __name__ == "__main__":
    app = QApplication()
    window = Ui_Doc_Obs_View_Action_Panel()
    window.show()
    app.exec()

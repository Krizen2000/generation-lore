...
class Ui_Favourite_View_Action_Panel(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setObjectName("Ui_Favourite_View_Action_Panel")
        self.load_widgets()
        self.set_layout()
        self.load_widget_assets()

        with open("src/genlore/ui_modules/favourite_view/ui_action_panel.qss", "r") as qss_file:
    ...
    def load_widgets(self):
        ...
        self.favourite_view_btn.setObjectName("favourite_view_btn")
    ...
    def load_widget_assets(self):
        ...
        mask = favourite_view_icon.createMaskFromColor(
            QColor("transparent"), Qt.MaskInColor)
        favourite_view_icon.fill(QColor("#E1E1E1"))
        favourite_view_icon.setMask(mask)
        self.favourite_view_btn.setIcon(favourite_view_icon)

        list_all_view_icon = QPixmap("icons/list.svg")
        mask = list_all_view_icon.createMaskFromColor(
            QColor("transparent"), Qt.MaskInColor)
        list_all_view_icon.fill(QColor("#FE5D9F"))
        list_all_view_icon.setMask(mask)
        self.list_view_btn.setIcon(list_all_view_icon)
...
if __name__ == "__main__":
    app = QApplication()
    window = Ui_Favourite_View_Action_Panel()
    window.show()
    app.exec()
